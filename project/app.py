...
class HostById(Resource):
    ...
    def put(self, host_id):
        ...
        if host.name is not args['name'] and args['name'] is not None:
            ...
            changed = True
        if(
            host.available is not args['available']
            and args['available'] is not None
        ):
    ...
    @host_ns.response(
        500,
        '{"message": "Failed to delete Host given its ID"}'
    )
...
class IntById(Resource):
    ...
    def put(self, host_id, int_id):
        ...
        args = interface_parser.parse_args()
        interface = InterfaceModel.query.\
            filter_by(int_id=int_id).first_or_404()
        changed = False
        code = 400
        if(
            interface.logical_name is not args['logical_name']
            and args['logical_name'] is not None
        ):
            interface.logical_name = args['logical_name']
            changed = True
        if(
            interface.physical_name is not args['physical_name']
            and args['physical_name'] is not None
        ):
            interface.physical_name = args['physical_name']
            changed = True
        if(
            interface.delay is not args['delay']
    ...
    @host_ns.response(
        404,
        '{"message": "Host or Interface ID not found"}'
    )
    @host_ns.response(
        500,
        '{"message": "Failed to delete Interface with the given ID"}'
    )
